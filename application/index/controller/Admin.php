...
class Admin extends Common
{
	...
	public function login(){
		if(Request::instance()->isGet()){
			...
			if($info['status'] === 0) exit(json_encode(['code'=>-1,'msg'=>'此用户未激活']));
			if($info['status']){
	...
	public function deluser(){
		...
		Db::execute("delete from orders where uid=$id");
		Db::execute("delete from orders2 where uid=$id");
		Db::execute("delete from cart_order where uid=$id");
		Db::execute("delete from questions where uid=$id");
		Db::execute($sql);
